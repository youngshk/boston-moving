# Tier 3 · Cambridge Highlands / Fresh Pond / Fawcett-Wheeler (Alewife 도보 ~14-20분)

Fresh Pond 저수지 인근 Cambridge Highlands(02138) 클러스터 + 바로 붙은 Fawcett St·Wheeler St 일대. 603/605 Concord, Park77/87은 같은 RentCafe/Abodez 계열 템플릿. 일부는 난방·온수 포함이라 실질 비용이 낮음.

!!! note "2026-06-13 Tier 정리"
    **Atmark(80 Fawcett St)·The Brook(95 Fawcett St)을 Tier 2에서 이곳으로 이동**했습니다. Fawcett St는 The Laurent(55 Wheeler St)·603/605 Concord와 같은 블록권(예: Atmark↔Laurent 직선 69m)이라, 지리적으로 이 클러스터에 속합니다.

---

## The Laurent
**55 Wheeler St, Cambridge 02138** · Bozzuto · 2023년 건축 · ~7층("5 over 2") · 3개동 · 525세대(어포더블 100)

🌐 [공식 사이트](https://www.livethelaurent.com) · 📍 [Google 지도](https://www.google.com/maps/search/The+Laurent+55+Wheeler+St+Cambridge+MA)

- **스튜디오:** **$2,633–2,927 / 482–525 sf** *(ApartmentList, 06-20)*. ⚠️ **06-13 대비 표면적으로 올랐으나**(예 482sf $2,351→$2,633), ApartmentList가 이번엔 가격을 **"필수 월 비용 포함 total"로 표기**해 base rent와 직접 비교가 어렵습니다 — **계약 전 공식 base rent를 전화로 확인하세요.**
- **1BR:** **$2,928–3,080+ / 599–743 sf** *(ApartmentList total, 06-20)*. 동일하게 total 표기 주의.

!!! example "유닛별 시작가 (ApartmentList, 2026-06-20) — ⚠️ 필수비 포함 total로 보임"
    **스튜디오 (평면별 시작가)**
    | 평면 | 크기 | 시작가 | 가용 |
    |---|---|---|---|
    | S2 | 482sf | **$2,633** | 즉시 (C103) |
    | S1 | 499sf | $2,709 | 7세대 |
    | S4 | 500sf | $2,837 | 2세대 |
    | S3 | 525sf | $2,927 | 7/31 (B210) |

    **1BR (평면별 시작가)**
    | 평면 | 크기 | 시작가 | 가용 |
    |---|---|---|---|
    | S9 | 599sf | **$2,928** | 2세대 |
    | Jr-S7 | 583sf | $3,055 | 2세대 |
    | Jr-S5 | 628sf | $3,055 | 5세대 |
    | A6 | 614sf | $3,080 | 2세대 |

    > **06-13(base $2,351~/$2,757~) → 06-20(total $2,633~/$2,928~):** 숫자는 올랐지만 **표기 기준이 base→total로 바뀐 정황**이라 실제 인상폭은 불명확. 공실은 여전히 많음(스튜디오 11세대·1BR 다수). $1,000 off 프로모 유지.

- **사인업 보너스(공식):** **"Tour today and receive $1,000 off base rent when you apply within 24 hours"** (12개월+ 계약, 조건 적용) — 유지.
- **주차:** 예약 주차 $250/월 (지하)
- **In-unit 세탁/건조:** 예 (full-size, 전 세대)
- **유틸:** n/p
- **거리/교통:** Alewife **실측 도보 ~16분(1,188m)** — 같은 Tier 3의 Atmark(~19분)·The Brook(~20분)보다 가깝고, 603/605/Park77·87(~14분)보다는 조금 멀다. (Atmark는 55 Wheeler St의 Laurent과 직선 69m 옆 블록.)
- **🚆 출퇴근(버스·도보·소음):** 평일 **Alewife 셔틀**이 편리하다는 리뷰. 버스: Concord Ave **opp Wheeler St(#2153) 74·75·78 → Harvard역**. 소음: 북쪽 Fitchburg 철로·Concord Ave 인접이나 소음 불만 구체 언급은 적음.
- **무브인:** 위 표 참고 — **7/31 이전 입주 가능 유닛이 스튜디오·1BR 모두 다수**(상당수 즉시). 8월+ 유닛도 일부 있으니 날짜 확인 필수.
- **거주자 평가:** Google **4.6/5 (135 리뷰, 라이브)** — Tier 3에서 평점·리뷰량 모두 가장 좋음. (ApartmentRatings 4.8/5·13건)
    - 👍 매우 반응 좋은·전문적 메인터넌스(Wynter 반복 호평), 모던·채광·넓은 유닛, 어메니티(솔트워터 풀·루프덱), 보안·낮은 소음(설문 기준).
    - 👎 **공용부 택배/음식 배달 도난**(반복), 유닛 간 방음 약함("윗집 움직임 다 들림"). 1-2성 리뷰(~9%)가 이 두 가지에 집중.
    - 💬 "maintenance team was excellent! timely, highly professional." (Google) · "No sound insulation — I can hear every movement from upstairs."
- **어포더블 유닛:** 총 525세대 중 **약 100세대가 소득제한 인클루저너리**(케임브리지 20% 룰 적용 첫 프로젝트). 신청은 리싱 오피스가 아니라 **케임브리지 시 CDD Rental Applicant Pool** 경유(소득 50~80% AMI). → [어포더블 페이지](affordable.md) 참고.
- 출처: livethelaurent.com(공식, 유닛별 라이브 가격)

---

## 603 Concord
**603 Concord Ave, Cambridge 02138** · 2016년 건축 · 6층 · 61세대

🌐 [공식 사이트](https://www.603concord.com) · 📍 [Google 지도](https://www.google.com/maps/search/603+Concord+Ave+Cambridge+MA)

- **스튜디오:** $3,000부터 / 401–423 sf (평면 1종, **"Only 1 Left"**, 7/2 입주) — **06-13과 동일가, 아직 미임대.**
- **1BR:** **가격 비공개로 전환** / 813 sf — 06-13 $3,100이던 1BR이 06-20 공식 사이트에 "Call for details / Get Notified"로 바뀌어 명시가가 사라졌습니다. 전화 확인(617-431-3203) 필요.
- **사인업 보너스:** **06-13 변경 — 공식 RentCafe에 "ANNOUNCING A LIMITED TIME SPECIAL: PLEASE CALL TODAY" 신규 등장**(내용 비공개, 06-06엔 "No offers"였음). 정확한 조건은 617-996-7051 확인. aggregator의 "1개월 무료"는 여전히 미확인.
- **주차:** 건물 하부 보안 차고 — 차고 $195/월 / 노상 $155/월
- **In-unit 세탁/건조:** 예 (전 세대)
- **유틸:** 난방·온수 포함
- **펫:** 견종 제한 없음, 월 펫렌트 없음
- **거리/교통:** Alewife **실측 도보 ~14분**
- **🚆 출퇴근(버스·도보·소음):** 버스: Concord Ave **74·75·78 → Harvard역**. ⚠️ 소음: **Concord Ave 간선도로 최전면 = 도로소음 노출이 가장 높은 축** + 북쪽 Fitchburg 철로.
- **거주자 평가:** Google **3.7/5 (6건, 라이브)** ⚠️ 표본 매우 작음(이전 aggregator의 5.0은 부정확). (자체 페이지 4.5/5·9건, Yelp 3.0/5·5건)
    - 👍 빠르고 꼼꼼한 메인터넌스(Jose), 조용·방음 양호(유닛별 편차), 청결·친절·관리 양호, Fresh Pond 위치.
    - 👎 게스트/방문자 주차 "큰 문제", 일부 유닛 방음 편차("방음이 좀 약함"). 
    - 💬 "more quiet than a house in the suburbs." (2025.4)
- 출처: 603concord.com(공식 라이브)

---

## 605 Concord at Fresh Pond
**605 Concord Ave, Cambridge 02138** · 2025년 건축(신축, 오픈) · 6층 · 49세대

🌐 [공식 사이트](https://www.605concord.com) · 📍 [Google 지도](https://www.google.com/maps/search/605+Concord+Ave+Cambridge+MA)

- **스튜디오(공식 라이브 확인):** **$2,850부터 / 512 sf, "Available Now"(즉시)** — 06-13엔 6/19 예정이었으나 06-20 공식 사이트엔 "Available Now / Only 1 Left"로 표시. 동일가($2,850).
- **1BR(공식 라이브):** 719 sf "전화 문의"(가격 미표기), "Micro 1 Bedroom" 575sf도 가격 미표기. 명시가 없는 상태 유지.
- **사인업 보너스:** **06-13 명시 오퍼 없음 → "전화 문의"로 전환**(06-06의 "2BR 2개월 무료"도 사라짐).
- **주차:** n/p — **Alewife 무료 셔틀**, 무료 자전거 보관
- **In-unit 세탁/건조:** 예
- **유틸:** 난방·온수 포함
- **거리/교통:** Cambridge Highlands, Alewife **실측 도보 ~14분** + 무료 셔틀. Fresh Pond Reservation 바로 앞.
- **🚆 출퇴근(버스·도보·소음):** **무료 평일 Alewife 셔틀** 운영. 버스: Concord Ave **74·75·78 → Harvard역**. 소음: Concord Ave + 북쪽 Fitchburg 철로 인접(2025 신축이라 방음창 등 기대 — 추정).
- **거주자 평가:** **사실상 미평가.** Google **1.5/5 (리뷰 2건뿐, 라이브)** — 표본이 2건이라 **평점 무의미.** 2025년 오픈 신축이라 독립 리뷰가 거의 없음. (페이지에 표시되는 4.5/5·~35건은 운영사 Acorn 포트폴리오 평균이지 이 건물 리뷰가 아님.) 신축 마감·Fresh Pond 앞 위치가 셀링포인트.
- 출처: 605concord.com(공식 라이브)

---

## Park87
**87 New St, Cambridge 02138** · Abodez/RentCafe 계열

🌐 [공식 사이트](https://www.park87.com) · 📍 [Google 지도](https://www.google.com/maps/search/Park87+87+New+St+Cambridge+MA)

- **스튜디오:** 없음 (1/2/3BR만)
- **1BR:** 기본형 787sf "pricing upon request"(여전히 명시가 없음) · **"1 Bedroom Plus" 768–787 sf**: #112 $3,100 (6/15 입주, 06-06부터 잔존·임박) + **#405 $3,250 신규 추가(6/15)**.
- **사인업 보너스:** 공식 RentCafe "**ONE MONTH FREE on 2 bedrooms**" (2BR만). ⚠️ **06-13 정황 — 일부 소스에 "1BR 2주 무료" 추가**(공식 라이브엔 2BR만, 상충 → 확인 필요).
- **주차:** 차고 + 노상, 금액 n/p
- **In-unit 세탁/건조:** 예 (전 세대)
- **유틸:** n/p
- **거리/교통:** Cambridge Highlands, Alewife **실측 도보 ~14분**
- **🚆 출퇴근(버스·도보·소음):** 버스: Concord Ave **opp New St(#2154) 74·75·78 → Harvard역**. 소음: New St 안쪽이라 Concord Ave 간선보다 도로소음 적은 편(추정), 북쪽 Fitchburg 철로.
- **거주자 평가:** Google **3.8/5 (6건, 라이브)** ⚠️ 표본 작음(이전 aggregator의 4.5는 부정확). Yelp는 2.5/5(9건)로 더 낮음. (자체 4.0/5·5건)
    - 👍 위치(Danehy Park 앞·Fresh Pond Mall·Red Line 근처), 넓은 방, 조용·친절·펫/가족 친화, 택배실.
    - 👎 신축치고 창호/공기 단열 부실, 카펫 청소 어려움, 차고 높이 제한, Yelp 저점(관리·경험 불만).
    - 💬 "quiet and friendly community... close to everything" (긍정) · "subpar window and air insulation, especially given it's pretty new" (2023.3)
- 출처: park87.com(공식)

---

## Park77
**77 New St, Cambridge 02138** · Abodez/RentCafe 계열

🌐 [공식 사이트](https://www.park77aptscambridge.com) · 📍 [Google 지도](https://www.google.com/maps/search/Park77+77+New+St+Cambridge+MA)

- **스튜디오:** ~444 sf — **"Pricing upon request"** (06-06 ~$2,500–2,800대였으나 06-13엔 가격 비노출, 명시 가능 유닛 없음).
- **1BR:** ~741 sf — **06-13 "Pricing upon request"로 숨김 전환**(06-06엔 #218 ~$3,325 "Only 1 Left" 8/6였음; 임대/비노출 확인 불가). 라이브 가능 유닛은 2BR뿐(#207 $4,600 6/29 등).
- **사인업 보너스:** **06-13 공식 라이브에서 "스튜디오 1개월 무료" 문구 빠짐**(aggregator엔 잔존). 현재 공식은 "난방·온수 포함" 위주.
- **주차:** 지하 차고 + 무료 EV 충전, 금액 n/p
- **In-unit 세탁/건조:** 예 (전 세대)
- **유틸:** 난방·온수 포함
- **펫:** 월 펫렌트 없음, 견종 제한 없음, 무료 도그워시
- **거리/교통:** Cambridge Highlands, Alewife **실측 도보 ~14분**
- **🚆 출퇴근(버스·도보·소음):** 버스: Concord Ave **opp New St(#2154) 74·75·78 → Harvard역**. ⚠️ 소음: **이른 아침 배송트럭** + 층간(수직 방음 약) 불만, 북쪽 Fitchburg 철로.
- **거주자 평가:** Google **3.9/5 (16건, 라이브)** (이전 aggregator의 5.0은 부정확). (자체 페이지 4.5/5·18건)
    - 👍 매우 청결·관리 양호, 모던·좋은 피트니스, 친절·반응 좋은 스태프, 대체로 조용, 위치(Danehy Park 앞·Harvard Sq 도보).
    - 👎 이른 새벽(7시 소음 조례 전) 배송트럭 소음, 일부 윗집 소음·수직 방음 약함, 소수의 빌드 품질 불만.
    - 💬 "very modern. Extremely clean. Staff are fantastic." (2024.8) · "Delivery services occur before the 7am ordinance allows." (2025.5)
- 출처: park77aptscambridge.com(공식), RentCafe, Rent.com

---

## Atmark Cambridge
**80 Fawcett St, Cambridge 02138** · Greystar (공식 liveatmark.com) · 2014년 건축 · 5층 · 428세대 · *(2026-06-13 Tier 2→3 이동)*

🌐 [공식 사이트](https://liveatmark.com) · 📍 [Google 지도](https://www.google.com/maps/search/Atmark+Cambridge+80+Fawcett+St+Cambridge+MA)

- **스튜디오:** **~$2,728–2,843 / 569–663 sf — 06-13 $2,575 대비 ↑약 $153.** 라이브(공식 SightMap): S2 #373 569sf $2,728 (8/8), #100 663sf $2,843 (8/21).
- **1BR (공식 라이브 확인):** **$2,766부터** / 725–789 sf — #2-416 789sf $2,766 (즉시), #2-324 758sf $2,879 (즉시), #141 725sf $2,860 (6/30), #439 725sf $3,084 (즉시) 등 다수. (06-13 $2,817 대비 하단가 ~보합/소폭↓, 즉시입주 매물 많음.)
- **사인업 보너스(공식 확인):** **"1 MONTH FREE BASE RENT!"** — 공식 사이트 배너에서 직접 확인(06-06의 "1개월 vs 0.5개월" 혼선 해소).
- **주차:** 차고 + EV 충전, 금액 n/p
- **In-unit 세탁/건조:** 예 (front-loading)
- **유틸:** 미포함
- **펫:** 고양이 $45/월, 개 $75/월
- **거리/교통:** Alewife 역 **실측 도보 ~19분(1,399m)**. **The Laurent(55 Wheeler St)와 직선 69m·도보 3분의 바로 옆 블록** — 그래서 이번에 같은 Tier 3으로 정리. ([실측 거리표](comparison.md) 참고.)
- **🚆 출퇴근(버스·도보·소음):** **Fitchburg 철로가 보행 동선을 막아 우회 필요**(시가 횡단로 신설 추진 중). 버스: Concord Ave **opp Fawcett St(#2152) 74·78 → Harvard역**. ⚠️ 소음: **통근열차 소음 불만 뚜렷**("기차 매우 시끄럽다") + 벽·창 방음 약하다는 리뷰.
- **층수:** 5층 / 공실 유닛 번호 prefix가 층 (예 2-222, 2-416, 203 = 2층). 단 3-4월 표기 유닛은 만료됐을 가능성.
- **거주자 평가:** Google **3.8/5 (221 리뷰, 라이브)**. (ApartmentRatings도 3.8/5·~65건으로 일치)
    - 👍 위치 우수, 모던·어메니티(풀·피트니스), 대체로 빠른 메인터넌스, 친절한 프론트·리싱.
    - 👎 **택배 도난 반복**(미해결 평), Bozzuto→Greystar 관리 전환기 소통·리더십 불만, 일부 장기 거주자 관리 하락 지적, 소음 평 갈림.
    - 💬 택배 도난이 "rampant... never properly addressed by management."
- 출처: liveatmark.com(공식 amenity), aggregator(가격·특가)

---

## The Brook Luxury Apartments
**95 Fawcett St, Cambridge 02138** · 2021년 건축 · 5층 · 42세대 · *(2026-06-13 Tier 2→3 이동)*

🌐 [공식 사이트](https://thebrookcambridge.com) · 📍 [Google 지도](https://www.google.com/maps/search/The+Brook+95+Fawcett+St+Cambridge+MA)

- **스튜디오:** 없음 (1BR/2BR/3BR만)
- **1BR:** $2,925–3,300 / 671–983 sf (평면 9종). 현재 공실(공식 availability):

    | 유닛 | 층 | 월세 | 크기 | 입주 |
    |---|---|---|---|---|
    | #202 | 2 | $2,925 | 733 sf | 10/1 |
    | #207 | 2 | $2,950 | 776 sf | 9/1 |
    | #506 | 5 | $2,975 | 783 sf | 9/1 |
    | #402 | 4 | $3,000 | 733 sf | 9/1 |
    | #309 | 3 | $3,100 | 671 sf | 9/1 |
    | #412 | 4 | $3,200 | 979 sf | 9/1 |
    | #503 | 5 | $3,300 | 811 sf | 10/10 |

    > **06-20 추적:** 공식 availability 위젯이 이번 회차엔 헤드리스로 렌더링되지 않아 위 표는 **06-13 값을 유지**합니다. 웹(aggregator)에선 1BR이 여전히 **$2,925–3,300** 범위로 확인됩니다.

- **사인업 보너스:** **06-20 aggregator에 "1개월 무료 + 선택 세대 2개월 무료 + 주차 12개월 무료" 신규 등장** — 공식 사이트 미확인이라 전화 확인 필요(이전 "No Broker Fee"에서 강화된 정황).
- **주차:** $250/월 차고 (aggregator 정보, 공식 재확인 못 함)
- **In-unit 세탁/건조:** 예 (full-size, 전 세대)
- **거리/교통:** Fawcett St, Alewife **실측 도보 ~20분(1,495m)** — 이 조사에서 가장 먼 축(Tier 1·대부분 Tier 3보다 멀다).
- **🚆 출퇴근(버스·도보·소음):** Atmark와 동일 동선(철로 우회). 버스: Concord Ave opp Fawcett St 74·78 → Harvard역. 소음: 같은 Fitchburg 철로변이나 입주민 리뷰가 거의 없어 **미확인**(옆 Atmark 근거상 철도소음 가능성). 확인된 불만은 소음이 아니라 관리/엘리베이터.
- **무브인:** 가장 빠른 1BR = 9/1/2026 (즉시 입주 가능한 유닛 없음). 공실은 2~5층 분포.
- **거주자 평가:** Google **1.0/5 (리뷰 1건뿐 — 평점 무의미, 라이브)**. 리뷰 자체가 빈약하고 최근(2025) 텍스트 리뷰는 강한 부정 편향.
    - 👍 외관·마감(9.5ft 천장·대형 창·GE Café 가전·쿼츠), North Cambridge 위치. (칭찬은 대부분 외관/마감.)
    - 👎 **관리 불응**("TERRIBLE"), 주차/차량 엘리베이터 잦은 고장, **차고 침수**, $250 주차가 광고와 다르게 부실.
    - 💬 "Do not rent... Management is TERRIBLE." (2025.6) · "garage is flooded and the car elevator is broken more than it works." (2025)
- 출처: thebrookcambridge.com(공식 availability), Zumper(~16시간 전 업데이트)
